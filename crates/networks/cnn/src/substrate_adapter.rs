//! Substrate + thalamic modulation helpers for the CNN (Phase
//! 11-substrate).
//!
//! The CNN is a feed-forward network with no membrane / tau dynamics,
//! so the substrate surface is narrower than the LNN's:
//!
//! - **Inference** — a thalamic attention scalar gates the input tensor
//!   and `dend.integration_efficiency` attenuates the output logits
//!   (signal fidelity falls as the substrate degrades).
//! - **Training** — `dend.plasticity_mod` scales the learning rate and
//!   `(ltp_capacity, ltd_capacity)` apply asymmetric gating to the
//!   sign of each gradient component.
//!
...
//! behaviour. The zero-cache sentinel (`DendriteSubstrateEffects::
//! is_zero_cache`) mirrors V1 commit `43785ee5e`: a stale-but-zeroed
//! cache falls back to base values rather than silently killing the
//! signal / learning.
...
/// Thalamic attention scalar applied to the whole input tensor.
///
/// - `None` channel → `1.0` (identity).
/// - [`nimcp_thalamic::RelayMode::Burst`] → `1.2` (burst relays
///   up-modulate afferents, matching the LNN convention).
...
/// multiply positive gradient components by `ltp_scale` and negative
/// ones by `ltd_scale`. Identity `(1.0, 1.0)` when `apply = false`,
/// `effects` is `None`, or the cache is zeroed.
#[must_use]
...
mod tests {
    ...
    #[test]
    fn integration_gain_identity_when_opted_out() {
        let mut eff = identity_effects();
        eff.1.integration_efficiency = 0.3;
    ...
    fn integration_gain_reads_dend_efficiency() {
        let mut eff = identity_effects();
        eff.1.integration_efficiency = 0.7;
        assert!((integration_gain(Some(&eff), true) - 0.7).abs() < 1e-6);
    }
    ...
    fn effective_lr_scales_with_plasticity() {
        ...
        assert!((effective_lr(0.02, Some(&eff), true) - 0.01).abs() < 1e-6);
    }
